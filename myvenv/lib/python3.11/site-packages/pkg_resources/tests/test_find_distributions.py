...
class TestFindDistributions:
    ...
    def test_zipped_sdist_one_level_removed(self, target_dir):
        shutil.copytree(TESTS_DATA_DIR / "my-test-package-zip", target_dir, dirs_exist_ok=True)
        dists = pkg_resources.find_distributions(str(target_dir / "my-test-package.zip"))
        assert [dist.project_name for dist in dists] == ["my-test-package"]
        dists = pkg_resources.find_distributions(str(target_dir / "my-test-package.zip"), only=True)
        assert not list(dists)
